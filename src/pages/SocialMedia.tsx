...
import { useSocialMediaList } from '@/queries/social-media'
...
const columnHelper = createColumnHelper<SocialMedia>()

const PLATFORM_SUGGESTIONS = ['facebook', 'instagram', 'tiktok', 'x', 'youtube', 'other']
...
const createInitialFormData = (siteId = ''): SocialMediaFormData => ({
  siteId,
  languageCode: '',
  platform: '',
  name: '',
...
function itemToFormData(item: SocialMedia): SocialMediaFormData {
  return {
    siteId: item.siteId ?? '',
...
): CreateSocialMediaRequest & UpdateSocialMediaRequest {
  return {
...
export function SocialMediaContent() {
  ...
  const [formData, setFormData] = useState<SocialMediaFormData>(createInitialFormData())
  ...
  const { data, isLoading, error } = useSocialMediaList({
    page,
    pageSize,
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Failed to create social media:', err)
    }
  }
  ...
  const columns = useMemo<ColumnDef<SocialMedia, any>[]>(
    () => [
      ...
      <div className="space-y-6">
        <div className="bg-card rounded-lg shadow-lg border border-border p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-2xl font-bold text-card-foreground">Social Media</h2>
              <p className="text-muted-foreground mt-1">Manage social media links</p>
            </div>
            <button
              onClick={() => {
          ...
          {isLoading ? (
            <div className="text-center py-12">
              ...
              <p className="mt-4 text-muted-foreground">Loading social media...</p>
            </div>
          ) : error ? (
            <div className="text-center py-12">
              <p className="text-red-500">Failed to load social media</p>
            </div>
          ) : !data?.data.length ? (
            <div className="text-center py-12">
        ...
        >
          <div className="p-6 space-y-4">
            <p className="text-card-foreground">
              Are you sure you want to delete "<strong>{selectedItem.name || selectedItem.platform}</strong>
...
function SocialMediaFormModal({
  ...
  title: string
  formData: SocialMediaFormData
  setFormData: React.Dispatch<React.SetStateAction<SocialMediaFormData>>
  ...
  const set = <K extends keyof SocialMediaFormData>(key: K, value: SocialMediaFormData[K]) =>
  ...
  return (
    <Modal title={title} onClose={onClose} maxWidth="max-w-2xl" scrollBody>
      <form onSubmit={onSubmit} className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-card-foreground mb-2">Platform *</label>
            <input
        ...
        <TextField label="Name" value={formData.name} onChange={(v) => set('name', v)} />
        <TextField label="URL" value={formData.url} onChange={(v) => set('url', v)} placeholder="https://..." />
